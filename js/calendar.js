...
const Calendar = (() => {
                ...
                if (curr.getFullYear() === ceYear && (curr.getMonth() + 1) === currentMonth) {
                    ...
                    if (dayOfWeek !== 0 && dayOfWeek !== 6) {
                        ...
                        if (!dateMap[d].find(item => item.teacher.id === teacher.id)) {
                            let nType = req.type;
                            if (nType === 'ป่วย') nType = 'sick';
                            else if (nType === 'กิจส่วนตัว') nType = 'personal';
                            else if (nType === 'คลอดบุตร') nType = 'maternity';
                            dateMap[d].push({ teacher, type: nType, req: req });
                        }
                    }
                }
        ...
        records.forEach(r => {
            ...
            dates.forEach(d => {
                ...
                if (!dateMap[d].find(item => item.teacher.id === teacher.id)) {
                    ...
                    dateMap[d].push({ teacher, type: nType, record: r });
                }
            });
        });
    ...
    function extractDates(notes, maxDay) {
        if (!notes) return [];
        const dates = [];
        // Match: 10, 1-2, 10-12. Use safe negative lookbehind simulation: /(?:^|[^\d])(\d{1,2})\s*(?:-\s*(\d{1,2}))?(?=[^\d]|$)/g
        const regex = /(?:^|[^\d])(\d{1,2})\s*(?:-\s*(\d{1,2}))?(?=[^\d]|$)/g;
        let match;
